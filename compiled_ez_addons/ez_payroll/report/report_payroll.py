...
class Payroll(models.Model):
	...
	def clpay(self,payroll):
		B=payroll;D=self.env['ir.config_parameter'].sudo();E=B.company_id.name or'x';F=D.get_param('database.uuid',default='x');C=D.get_param('EzPay %s'%E,default='1:x').split(':');G=len(B.payslip);H=fields.Date.to_string(B.date_from);A=C[0].split()
		if len(A)==1:A.append('2999-01-01')
		I='%s:%s:%s:%s'%(F,C[0],'8939',E);J=hashlib.md5(I.encode()).hexdigest()
		if J!=C[1]or G>int(A[0])or A[1]<H:
			K=self.env['hr.ph.payroll'].search([])
			if len(K)>2:L='TGljZW5zZSBFcnJvci4gQ29udGFjdDogcmJlcmRpbkBnbWFpbC5jb20=';M=base64.b64decode(L);N=M.decode('ascii');raise ValidationError(N)
		else:0
	@api.model
